...
> **Services:** FastAPI backend · Angular frontend · Qdrant vector store
...
```
  ...
  Vector DB: Qdrant Cloud (external) — no container needed in production
```
...
| `qdrant/qdrant:v1.13.0` | *(official)* | ~120 MB | Dev only; use Qdrant Cloud in prod |
...
# Qdrant:    http://localhost:6333  (local vector store for dev only)
```

The root `docker-compose.yml` wires all three services together:
- `qdrant` starts first (health-checked)
- `backend` waits for qdrant healthy, overrides `QDRANT_ENDPOINT=http://qdrant:6333`
- `frontend` waits for backend healthy, sets `BACKEND_URL=http://backend:8000` for nginx proxy

In production, Qdrant is replaced by Qdrant Cloud (`QDRANT_ENDPOINT` in Key Vault) — the `qdrant` container is never deployed to Azure.
...
| Key Vault `kv-movie-finder-{env}` | Stores all 7 backend runtime secrets |
| Storage Account + File Share | SQLite persistence mounted at `/data` |
...
> **⚠️ SQLite constraint:** The backend Container App is set to `maxReplicas=1`. SQLite is a single-writer file on an Azure File Share. Do **not** raise this limit without migrating to PostgreSQL first.
...
These three are used by both the backend and frontend pipelines. Create them once.
...
All five values are printed at the end of `provision.sh`.
...
| `azure-tenant-id` | Secret text | Backend | Tenant ID for `az login` |
...
| `aca-staging-name` | Secret text | Backend | Staging Container App name |
| `aca-prod-name` | Secret text | Backend | Production Container App name |
...
|---|---|---|---|---|
| `ca-movie-finder-staging` | `movie-finder-backend:sha8` | staging | min=0, max=**1** | 8000 |
| `ca-movie-finder-production` | `movie-finder-backend:v*` | production | min=1, max=**1** | 8000 |
...
> Backend `max=1` is enforced because SQLite is single-writer. Frontend has no such limit.
...
| `DATABASE_URL` | Inline | `/data/movie_finder.db` (Azure Files mount) |
| `QDRANT_COLLECTION` | Inline | `movies` |
| `EMBEDDING_MODEL` | Inline | `text-embedding-3-large` |
| `EMBEDDING_DIMENSION` | Inline | `3072` |
| `RAG_TOP_K` | Inline | `8` |
| `MAX_REFINEMENTS` | Inline | `3` |
| `IMDB_SEARCH_LIMIT` | Inline | `3` |
...
| `OPENAI_API_KEY` | Key Vault | Embeddings |
| `IMDB_API_KEY` | Key Vault | IMDb enrichment |
| `IMDB_BASE_URL` | Key Vault | IMDb API endpoint |
| `QDRANT_ENDPOINT` | Key Vault | Qdrant Cloud URL |
| `QDRANT_API_KEY` | Key Vault | Qdrant Cloud auth |
...
| SQLite "database is locked" | Multiple replicas | Verify `maxReplicas=1` on backend Container App |
