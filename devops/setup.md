...
Infrastructure as Code via Terraform is the provisioning path
(see [ADR-0006](../architecture/decisions/0006-terraform-iac.md)). The
`infrastructure/` submodule owns Azure resources, Key Vault secret names,
Container Apps environment variables, and provider/vector-store validation.

### 5.1 Terraform apply

The parent `movie-finder` CI pipeline runs `cd infrastructure && make apply`
before rolling Container App images. Manual operators use the same Docker-backed
target with `TF_VAR_*` environment variables.
...
export TF_VAR_backend_image_tag="<sha8-or-release-tag>"
export TF_VAR_frontend_image_tag="<sha8-or-release-tag>"
export TF_VAR_app_secret_key="$(openssl rand -hex 32)"
export TF_VAR_db_admin_username="moviefinderadmin"
export TF_VAR_db_admin_password="<strong-password>"
export TF_VAR_vector_store=qdrant
export TF_VAR_vector_collection_prefix=movies
export TF_VAR_qdrant_url="https://your-cluster.qdrant.io"
export TF_VAR_qdrant_api_key_ro="..."
...
| Service Principal                                   | Created separately; Jenkins/GitHub use it to push images and run Terraform |
...
Jenkins and GitHub Actions credentials must be created before CI can apply
Terraform. Use the canonical names in
`infrastructure/docs/provider-runtime-contract.md`.
...
```bash
# Variables from Terraform outputs or Azure Portal
RG="movie-finder-rg"
ACA_ENV="movie-finder-env-staging"
...
### 5.4 Grant CI service principal access

The CI service principal needs Contributor on the environment resource groups:

```bash
SP_APP_ID="<CI service principal appId>"
...
> Use the ACR login server from Terraform outputs or the Azure Portal.
...
These backend-specific credentials are no longer used by the canonical root
pipeline. The root pipeline applies Terraform and deploys both apps together.
...
Terraform stores these when the corresponding `TF_VAR_*` values are provided:
...
| `az login` fails            | CI service principal wrong or expired | Re-create the SP and update CI credentials              |
